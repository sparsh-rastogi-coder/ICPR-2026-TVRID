...
# IMPORT FROM models3 to get the fixed class
...
def get_args():
    ...
    parser.add_argument("--track", choices=TRACKS.keys(), default="depth")
    ...
    # Default is OSNet to match your checkpoint
    parser.add_argument(
        "--backbone", 
        choices=["osnet", "resnet", "vit", "convnext"], 
        default="osnet", 
        help="Force backbone type (overrides checkpoint)"
    )
...
def main():
    ...
    load_overrides = {}
    if args.backbone:
        load_overrides["backbone_type"] = args.backbone

    # Loading the model (classifier head is loaded but ignored during embed_dataset)
    model = ReIDLightning.load_from_checkpoint(args.checkpoint, **load_overrides)
    ...
    print(f"Loaded model backbone: {model.hparams.backbone_type}")
